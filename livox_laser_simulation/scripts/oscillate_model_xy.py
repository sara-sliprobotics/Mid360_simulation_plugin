#!/usr/bin/env python3
"""Move the sensor model back and forth along X and/or Y, publishing odom TF."""
...
def main():
    ...
    x_min = rospy.get_param('~x_min', 0.0)
    x_max = rospy.get_param('~x_max', 0.0)
    y_min = rospy.get_param('~y_min', 0.0)
    y_max = rospy.get_param('~y_max', 0.0)
    period = rospy.get_param('~period', 6.0)  # seconds for one full cycle
    phase_offset = rospy.get_param('~phase_offset', 0.0)  # radians; pi/2 starts at max
    ...
    rospy.loginfo(f"Oscillating '{model_name}' on X: [{x_min}, {x_max}], Y: [{y_min}, {y_max}], period={period}s")

    while not rospy.is_shutdown():
        t = (rospy.Time.now() - start_time).to_sec()
        phase = 2.0 * math.pi * t / period + phase_offset
        x = x_offset + x_amplitude * math.sin(phase)
        y = y_offset + y_amplitude * math.sin(phase)
        vx = x_amplitude * (2.0 * math.pi / period) * math.cos(phase)
        vy = y_amplitude * (2.0 * math.pi / period) * math.cos(phase)
